...
# Database setup
conn = sqlite3.connect('Data/person_data.db', check_same_thread=False)
db_cursor = conn.cursor()

db_cursor.execute('''
CREATE TABLE IF NOT EXISTS person_log (             
    total_person INTEGER PRIMARY KEY AUTOINCREMENT,
    person_id INTEGER,
    status TEXT,
    timestamp TEXT
)
...
def view_log():
    ...
    df = pd.DataFrame(data, columns=["total_person","person_id", "status", "timestamp"])
    return df
